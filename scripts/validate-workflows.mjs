...
if (agentWorkflow) {
  ...
  check(
    ...
    "Agent: start_domain_research must stay the free fallback and ask no ownership question",
  );
  ...
  check(
    getPaidResearchTool?.parameters?.workflowId?.value ===
      ...
      /read-only source of truth for saved paid SEO snapshots/i.test(
  ...
  check(
    ...
    "Agent: get_seo_article must remain a conversation-bound read",
  );
...
if (startResearchWorkflow) {
  ...
  check(
    ...
    "start_domain_research must validate a direct public-domain research request",
  );
  check(
