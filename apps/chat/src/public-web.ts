...
/**
 * Fetch one generated image over the same DNS-safe, HTTPS-only path the research
 * reader uses. Provider URLs expire, and the chat page may only load same-origin
 * images, so the bytes are pulled once and stored locally rather than hot-linked.
 */
export async function fetchPublicImage(value: string): Promise<PublicImage> {
    ...
    const result = await new Promise<{
      ...
      const request = httpsRequest(
        ...
        (upstream) => {
          ...
          if ([301, 302, 303, 307, 308].includes(statusCode) && location) {
            upstream.resume();
            resolveImage({ statusCode, location, contentType: "", chunks: [] });
            return;
          }
          const contentType = (String(upstream.headers["content-type"] ?? "").split(";")[0] ?? "")
            .trim()
            .toLowerCase();
          const chunks: Buffer[] = [];
          let bytes = 0;
          upstream.on("data", (rawChunk: Buffer | string) => {
            ...
            if (bytes > MAX_IMAGE_BYTES) {
              upstream.destroy();
              rejectImage(new Error("The generated image is too large to save"));
              return;
            }
            chunks.push(chunk);
          });
          upstream.on("end", () => resolveImage({ statusCode, contentType, chunks }));
          upstream.on("error", rejectImage);
        },
      );
      request.on("timeout", () => request.destroy(new Error("The image request timed out")));
      request.on("error", rejectImage);
      request.end();
    });

    if (result.location !== undefined) {
      if (redirects === MAX_REDIRECTS) throw new Error("The image redirected too many times");
      current = validatePublicUrl(new URL(result.location, current).toString());
      continue;
    }
    if (result.statusCode !== 200) {
      throw new Error(`The image could not be read (status ${result.statusCode})`);
    }
    const extension = ALLOWED_IMAGE_TYPES[result.contentType];
    if (extension === undefined) {
      throw new Error("Only PNG, JPEG and WebP images can be saved");
    }
    const bytes = Buffer.concat(result.chunks);
    if (bytes.length === 0) throw new Error("The image was empty");
    return { requestedUrl: current.toString(), contentType: result.contentType, extension, bytes };
  }
